class Board:
    ...
    def _init_board(self):
        ...
        Return:
            list: Двумерный список, представляющий игровое поле.
        """
    ...
    def parse_position(self, pos):
        ...
        Args:
            pos (str): Позиция в нотации (например, 'e2').

        Return:
            tuple: Кортеж (индекс строки, индекс столбца).
        """
    ...
    def make_move(self, start, end):
        ...
        # Для шашек: если перепрыгиваем через фигуру, то убираем её
...
class Piece:
    ...
    def __init__(self, color, position):
        """
        Инициализирует фигуру, устанавливая цвет и позицию.
    ...
    def is_valid_move(self, board, end):
        ...
        Args:
            board (Board): Экземпляр доски.
            end (str): Конечная позиция.

        Return:
            bool: Корректен ли ход.
        """
        pass
    ...
    def get_possible_moves(self, board):
        ...
        Return:
            list: Список строк, представляющих допустимые ходы в нотации.
        """
        pass
...
class Checker(Piece):
    ...
    def is_valid_move(self, board, end):
        """
        Проверяет, является ли ход корректным для обычной шашки.
    ...
    def get_possible_moves(self, board):
        ...
        Return:
            list: Список допустимых ходов в нотации.
        """
        moves = []
        s_row, s_col = board.parse_position(self.position)
        direction = -1 if self.color == 'white' else 1
...
class KingChecker(Piece):
    ...
    def is_valid_move(self, board, end):
        """
        Проверяет, является ли ход корректным для дамки в шашках.
...
class Pawn(Piece):
    ...
    def is_valid_move(self, board, end):
        """
        Проверяет, является ли ход корректным для пешки.
    ...
    def get_possible_moves(self, board):
        ...
        if 0 <= s_row + direction < 8 and board.board[s_row + direction][s_col] == '.':
            ...
            # Два шага вперёд с начальной позиции
...
class Knight(Piece):
    ...
    def is_valid_move(self, board, end):
        """
        Проверяет, является ли ход корректным для коня.
...
class Bishop(Piece):
    ...
    def is_valid_move(self, board, end):
        """
        Проверяет, является ли ход корректным для слона.
...
class Rook(Piece):
    ...
    def is_valid_move(self, board, end):
        """
        Проверяет, является ли ход корректным для ладьи.
...
class Queen(Piece):
    ...
    def is_valid_move(self, board, end):
        """
        Проверяет, является ли ход корректным для ферзя.
...
class King(Piece):
    ...
    def is_valid_move(self, board, end):
        """
        Проверяет, является ли ход корректным для короля.
...
class Wizard(Piece):
    ...
    def is_valid_move(self, board, end):
        """
        Проверяет, является ли ход корректным для волшебника.
...
class Dragon(Piece):
    ...
    def is_valid_move(self, board, end):
        """
        Проверяет, является ли ход корректным для дракона.
...
class Archer(Piece):
    ...
    def is_valid_move(self, board, end):
        """
        Проверяет, является ли ход корректным для стрелка.
        ...
        # "Выстрел" на две клетки по диагонали
...
class Game:
    ...
    def play(self):
        ...
        В рамках цикла обрабатываются команды пользователя:
        - ходы (например, 'e2 e4'),
        - команды отмены, повтора, подсказки, угроз, сохранения, загрузки и выхода.
        """
        while True:
...
class CheckersGame(Game):
    ...
    def is_valid_move(self, start, end):
        ...
        Args:
            start (str): Начальная позиция (например, 'b2').
            end (str): Конечная позиция (например, 'b3').
    ...
    def make_move(self, start, end):
        """
        Выполняет ход в шашках, включая возможное взятие.
        ...
        # Проверяем, убираем ли шашку соперника при перепрыгивании
        if abs(s_row - e_row) == 2:
            mid_row = (s_row + e_row) // 2
            mid_col = (s_col + e_col) // 2
    ...
    def hint(self, pos):
        """
        Выводит все возможные ходы для шашки на указанной позиции.

        Args:
            pos (str): Позиция шашки (например, 'b2').
        """
        row, col = self.board.parse_position(pos)
        piece = self.board.board[row][col]
    ...
    def threats(self, pos):
        """
        Отображает шашки, которые угрожают указанной клетке.

        Args:
            pos (str): Позиция клетки (например, 'b3').
        """
        row, col = self.board.parse_position(pos)
        threats_list = []
        for i in range(8):
